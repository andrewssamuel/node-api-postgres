...
const getpurchaseorderdetailsbymonth = (request, response) => {
  str_query = "select one.monthyear, two.ord_quantity as ord_quantity_2019, one.store_name, one.product_name, one.ord_quantity, one.ord_quantity as ord_qty_2019, one.store_id, one.product_id, one.invoice_number, one.forcasted_qty, one.orderedflag, one.override from (select to_char(to_date(CAST(CONCAT(CAST(year AS text), '/', CAST(month AS text), '/', '01') as text),'YYYY/mm/dd'), 'Mon-YYYY') as monthyear, sm.store_name,pm.product_name, po.ord_quantity,  po.store_id, po.product_id, po.invoice_number, po.month, po.forcasted_qty, po.orderedflag,fo.override from purchase_order po, store_master sm, product_master pm, forcase_override fo where po.store_id = sm.store_id and po.product_id = pm.product_id and po.store_id=fo.store_id and po.product_id = fo.product_id and po.predictedflag is true) one, (select to_char(to_date(CAST(CONCAT(CAST(year AS text), '/', CAST(month AS text), '/', '01') as text),'YYYY/mm/dd'), 'Mon-YYYY') as monthyear, sm.store_name,pm.product_name, po.ord_quantity,  po.store_id, po.product_id, po.invoice_number, po.month, po.forcasted_qty, po.orderedflag,fo.override from purchase_order po, store_master sm, product_master pm, forcase_override fo where po.store_id = sm.store_id and po.product_id = pm.product_id and po.store_id=fo.store_id and po.product_id = fo.product_id and po.year = 2019) two where one.product_id = two.product_id and one.store_id = two.store_id and one.month = two.month"
  pool.query(str_query,(error, results) => {
    if (error) {
      throw error
    }
